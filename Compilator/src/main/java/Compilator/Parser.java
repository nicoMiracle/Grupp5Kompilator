...
public class Parser {
    ...
    public StatementNode parseStatementNode() {
        if(tokens.get(position).type().equals(TokenType.CONTROL_IF)){
    ...
    private boolean findEqual(){
        for(int i = position; i < tokens.size();i++){
            TokenType token = tokens.get(i).type();
    ...
    public TermNode parseTermNode(){
        ...
        }else if (position < tokens.size() && tokens.get(position).type().equals(TokenType.IDENTIFIER)) {
            IdentifierNode identifierNode = parseIdentifierNode(tokens.get(position).lexem());
    ...
    private PositiveTermNode parsePositiveTerm(){
        ...
        if(termNode.getIdentifierNode()!=null){
            ...
            throw new ParseException("Invalid token efter Plus: " + tokens.get(position).lexem() + " on line " + tokens.get(position).line());
        }
    }
    private NegativeTerm parseNegativeTerm(){
        ...
        if(termNode.getIdentifierNode()!=null){
            ...
            throw new ParseException("Invalid token efter Minus : " + tokens.get(position).lexem() + " on line " + tokens.get(position).line());
        }
    }
